...
import { badRequest, internalServerError, notFound, ok } from "https://ghuc.cc/worker-tools/response-creators/index.ts";
...
const router = new WorkerRouter()
    ...
    .get("/iris/timetable/:eva", middleware, async (_, { params, searchParams }) => {
        if (isNaN(parseInt(params.eva!))) return badRequest("EVA is not a number")
        if (!searchParams.get("start") || new Date(parseInt(searchParams.get("start")!)).toString() == "Invalid Date")
        ...
        const result = await iris(parseInt(params.eva!), {
            ...
            endDate: searchParams.get("end") ? new Date(parseInt(searchParams.get("end")!)) : undefined,
